// Headless JS probe of one scene (dev server must be running):
//   EVAL="<expression>" node tools/probe.mjs <sceneId> x [js-before] [waitMs]
...
try {
  ...
  const { result } = await send("Runtime.evaluate", { expression: process.env.EVAL || "1", returnByValue: true, awaitPromise: true }, sessionId);

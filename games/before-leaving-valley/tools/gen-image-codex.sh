...
SCR="${TMPDIR:-/tmp}/blv-gen"; mkdir -p "$SCR/codex"
WORK=$(mktemp -d "$SCR/codex/job.XXXXXX")
OUTWIN=$(cygpath -w -a "$OUT")
PNG="$WORK/out.png"; PNGWIN=$(cygpath -w -a "$PNG")
case "$ASPECT" in
...
mkdir -p "$(dirname "$OUT")"
( cd "$WORK" && timeout 480 "$CODEX" exec --skip-git-repo-check -s workspace-write -C "$(cygpath -w -a "$WORK")" "${REFARGS[@]}" - < "$WORK/msg.txt" > "$WORK/result.txt" 2> "$WORK/err.txt" )
if [ -s "$PNG" ]; then
  case "$OUT" in
    *.png|*.PNG) cp "$PNG" "$OUT" ;;
